...
This file tells agents how to act in this repo. It is deliberately **portable** — it contains only generic rules that apply to any project using this template. When solutions compete, prefer: **secure › correct › reliable › reversible › simple › resource-efficient › maintainable**. Inspect before changing; never assume when you can look. Read it before making changes. For this project's layout, deployment commands, protected resources, per-service facts, rationale, and operational gotchas, see `docs/GUIDE.md` (the only place project-specific facts live). For open problems, see `docs/ISSUES.md`.
...
5. **Minimize token usage.** Be extremely efficient: short, accurate, and understanding. No filler, no preamble, no restating the question, no recaps of what you just did. Batch tool calls that can run in parallel. Read only the file regions you need. Prefer one precise edit over rewriting whole sections. Answer in as few words as the task allows without sacrificing correctness: at most two sentences of prose unless the task itself needs a list, a table or code, and when code or a single value is asked for, return the raw block only — no preamble, no recap of work already done, no restating of the request. Outside this repo the same caps are enforced every turn by the host's goose briefing (`config/goose/briefing.md`).
...
9. **Never read secrets, credentials, or sensitive user data.** Do not read, print, or inspect database files, password-manager data, user files, uploaded files, `.env` files, SSH private keys, TLS cert/key files, identity state, or any file containing credentials, tokens, passwords, or personal data. Never reproduce a secret's *value* in a response, log, commit, or generated doc — refer to where it lives, and redact it if it appears in command output. The one exception is a credential a command has *just generated*: its delivery is the terminal, so print it once at creation — and never write it to a file (`make kuma-passwd` / `make mail-password` print; nothing is stored on disk). Reading these is a privacy violation — treat them as untouchable. If a task requires knowing a value from a secret file, ask the operator to provide it instead.
...
16. **Change procedure for anything that can affect a running service.** (1) inspect the current config, (2) understand its dependencies, (3) name the likely failure modes, (4) establish a rollback (backup / tag / revert recipe, rule 10), (5) make the smallest change, (6) validate before applying, (7) apply, (8) verify the service is healthy afterwards. One logical change at a time. Rule 10 is the network-facing instance of this.
17. **Pause and reassess before irreversible or high-impact actions.** Deleting data, databases or volumes; removing users; changing SSH/sudo/firewall/DNS/TLS/auth; changing kernel or system settings; exposing a service publicly; rewriting Git history; destroying infrastructure. These need explicit operator intent — never infer them from a vague request.
18. **Least privilege.** Use root only when the task needs it; never broaden permissions (sudoers, container capabilities, file modes, firewall) to work around a temporary problem.
19. **Dependencies.** Prefer something already present, or the official package/repo. Never pipe a remote script into a shell when a signed package exists. Know what a dependency is, where it comes from and what it changes before adding it.
20. **No fabrication.** Never report a change, test, or result you did not actually produce. If a test could not be run, say so and why.
21. **Deep scans and incidents** follow `docs/DEBUG.md` — read-only first, layer ladder, one change at a time, verify with a probe.
...
13. **Docs record the setup, never what the operator does with it.** Never write volatile, operator-editable state into any `.md`: statistics, database contents, user lists, mailbox addresses in use, which app is running, web-UI tuning decisions pending, quota values that change from a website, monitor definitions, anything the operator can change from a web UI without touching the host terminal. If a fact can change via a website, it will change, and the doc will be stale the moment it does. Setup facts (how a service is deployed, which routes exist, which config files matter) belong in docs; *usage* of the services does not.
...
15. **Image files are opt-in, per task, on-the-go.** Never open, view, or otherwise inspect image files (jpg/png/webp/gif/etc.) by default — no image viewers, thumbnails, screenshots, or OCR. Images can carry personal data and are usually unnecessary for repo work. The only exception is an **explicit, task-scoped authorization from the operator** given while the task is in progress (e.g. "review the screenshots of the live page" or "look at this image I uploaded"). A standing instruction to inspect images is never implied — each authorization applies to the images and task it names, and expires with that task. Screenshots of the operator's own public pages for layout review are covered by this rule too: only read them once the operator has said so for that run. Without authorization, if a task depends on what an image shows, ask the operator to describe it instead.
...
- **`docs/DEBUG.md`** — the diagnostic runbook: how to deep-scan or triage an incident on this system (read-only first, layer ladder, verification probes, report template). Procedures only — the rules are here, the project facts in `docs/GUIDE.md`.
...
13. **Never write volatile, operator-editable state** (rule 13 above): statistics, users, mailbox contents, monitor definitions, website-editable config values. Docs describe the setup, not the usage.
