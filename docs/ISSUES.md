...
- **`tail.$DOMAIN` mini terminal + gated shell** — unauthenticated command terminal at `/` (`go <vhost> [page]`, live prediction over a catalogue generated from the vhost files by `make tail-targets`) and `basic_auth` (user `kefohaine`) on `/ttyd`; `make tail-auth` sets/rotates it and prints the password once.
