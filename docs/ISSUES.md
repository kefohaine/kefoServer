...
- **Opt-in GitHub remote access (2026-09-25)** — the installer's last prompt asks (explicit `true`/`false`) whether to configure access to the operator's own repos; `true` mints `/root/.ssh/github_key`, prints the public key with its GitHub location, waits until GitHub accepts it, then pushes.
- **SSH key prompt is now required up front (2026-09-25)** — the installer asks for the operator device's public key (strict: junk and private keys rejected, re-asks), installs it before any phase runs, so sshd hardening applies in the same run; `ssh_keys` stays only as a safety-net tag.
- **Post-install optimize.sh offer (2026-09-25)** — once every error tag is green the installer asks yes/no whether to run the VPS performance pass; declining prints the command to run it later.
- **`make render` defeats the installer's "create the .env if absent" guards (2026-09-25)** — `modules/nextcloud/.env` and `modules/vaultwarden/.env` are both created by `make render` (instance-values block) long before `seed_stack` runs, so its `[ ! -f … ]` guards never fired: compose interpolated `NEXTCLOUD_ADMIN_USER/PASSWORD` and every `SMTP_*` as EMPTY. Consequence chain: the Nextcloud image entrypoint refused to install (`Set an admin Login.` → Nextcloud never installed → every occ step in `nextcloud_setup` skipped), Vaultwarden crash-looped on `SMTP_SECURITY is invalid`, and `vaultwarden_setup` died on an unbound `$SMTP_PASSWORD` (the unbound expansion killed only the pipeline subshell, so the summary blamed the mailbox). Both seeds are now PER KEY (`grep -q '^KEY=' || append`).
- **Nextcloud can stay uninstalled forever after one failed first boot (2026-09-25)** — the image entrypoint installs only while `/var/www/html` has no `version.php`; a first attempt that fails (empty admin creds, DB not up yet, CT/image stall) still leaves `version.php`, so every later start reads "already at the image version" and silently skips BOTH install and upgrade — the container reports `healthy` while `occ status` says `installed: false`, and no config/Talk/SMTP/cron wiring ever runs. `nextcloud_setup` now runs `occ maintenance:install` itself (same options, from `.env`) whenever occ reports not-installed, then sets `trusted_domains` (occ does not read `NEXTCLOUD_TRUSTED_DOMAINS` — only the entrypoint does).
- **The edge could not start at all (2026-09-25)** — `${DATA_DIR}/www` was bind-mounted at `/etc/caddy/www`, nested INSIDE the read-only `rendered/caddy:/etc/caddy` mount. runc cannot `mkdir` a mountpoint under an already-mounted read-only filesystem: `error mounting "…/data/www" to rootfs at "/etc/caddy/www" … mkdirat …/merged/etc/caddy/www: read-only file system`, so the container sat at `Created`, Caddy never listened, ACME never ran and all six `cert_*` checks failed with an empty issuer (one bug, seven error tags; pre-creating the dir in the image does not help). The web root is now a plain sibling path, `/srv/www` — compose, both vhosts and the docs.
- **`make talk-gen` aborted AFTER writing the secrets (2026-09-25)** — the render read `$REPO/repo/modules/nextcloud/talk/*.example`, but `$REPO` is `$DATA_DIR` in that script, so the path never existed (`data/repo/…`); `set -e` then reported "stack secrets not generated" although the secrets were fine, and left a 0-byte `data/talk/server.conf` (talk-hpb: `open /config/server.conf: permission denied`) plus `data/talk/turnserver.conf` as a DIRECTORY (docker creates a dir as the source of a missing bind mount). Paths now derive from `$REPO_DIR`, the templates are checked up front, and a dir placeholder is cleared before writing.
- **Uptime Kuma 2.5 never created its schema (2026-09-25)** — Kuma 2.5+ boots into its "Setup Database" web wizard (port open and healthcheck green, `kuma.db` left at 0 bytes, no tables) unless it finds `db-config.json`/`kuma.db` or `UPTIME_KUMA_DB_TYPE`. `kuma_seed` polled for the FILE, timed out, and `sqlite3` then created an empty db → `no such table: docker_host/monitor/group`. The compose now sets `UPTIME_KUMA_DB_TYPE=sqlite` and the seed waits for the `user` TABLE (5 min) instead of the file.
- **Same run, four smaller defects (2026-09-25)** — `fail2ban` was flagged 0.3 s before install-config's own restart brought the jail up (now polled for 30 s, so the tag cannot fire on an active server); the ACME re-attempt ran `docker restart "$DOMAIN"` while the container is named `caddy` (`No such container: fxmq.net` → the retry never happened); `nextcloud_setup` gave up after 3 min (now 8 min with progress lines to `$LOG`); and each mailbox was created with a single un-retried `setup email add` even though docker-mailserver shuts down ~2 min after start when no account exists and compose restarts it — one attempt could land in a dead window and both senders stayed uncreated (`mailbox_add` now retries for 5 min).
