...
|---|---|---|
| Templates: `config/**`, `services/caddy/**`, docs, `recipes/**` | `{{TOKEN}}` + `scripts/render.sh` | `data/instance.conf` |
| Compose files (`services/*/docker-compose*.yml`) | `${VAR}` (docker compose substitution) | the instance block `make render` writes into `services/*/.env` |
| Scripts (`scripts/**`) | plain shell variables, never tokens | `scripts/instance.sh`, sourced from `data/instance.conf` |
...
lives. `scripts/install.sh` writes it when it prompts; it is plain `KEY=VALUE`
and safe to edit by hand (`make render` afterwards).

```
scripts/render.sh --tokens        # every token, its current value, and what it means
```
...
`scripts/stack-up.sh` renders first, in every mode, so a fresh clone, a
...
3. use `{{NAME}}` in a template, `${NAME}` in a compose file, or `$NAME` in a
   script (after sourcing `scripts/instance.sh`)
...
`scripts/migrate-layout.sh` (kept for the historical 2026-09-22 layout move)
...
`make render` appends a marked block to each `services/*/.env`:
