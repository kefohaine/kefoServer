...
#
#   bash scripts/install/migrate-layout.sh --dry-run     # print every step, change nothing
#   bash scripts/install/migrate-layout.sh --yes         # do it
#   bash scripts/install/migrate-layout.sh --purge-op    # after root login is verified:
...
edge_config_ok() {
  docker compose -f "$REPO_NEW/modules/$DOMAIN_CTN/docker-compose.yml" config -q 2>/dev/null || return 1
  docker run --rm --entrypoint caddy \
    -v "$REPO_NEW/modules/$DOMAIN_CTN:/etc/caddy:ro" "$DOMAIN_CTN:local" \
...
stack_up() {
  ...
  for f in "$REPO_NEW"/modules/*/docker-compose.yml "$REPO_NEW"/modules/*/docker-compose.db.yml; do
    [ -f "$f" ] || continue
  ...
  if ! run compose_up "$REPO_NEW/modules/$DOMAIN_CTN/docker-compose.yml" --build; then
    ...
    if [ "$DRY" = 0 ] && docker image inspect "$DOMAIN_CTN:rollback" >/dev/null 2>&1; then
      ...
      compose_up "$REPO_NEW/modules/$DOMAIN_CTN/docker-compose.yml"
    fi
...
SMOKE_HOST=$(ls "$REPO_OLD/modules/$DOMAIN_CTN/vhosts/" 2>/dev/null \
  | sed -n 's/^\(www\..*\)\.caddy$/\1/p' | head -1)
[ -n "${SMOKE_HOST:-}" ] || SMOKE_HOST=$(ls "$REPO_OLD/modules/$DOMAIN_CTN/vhosts/" 2>/dev/null \
