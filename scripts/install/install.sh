...
# read scripts/install/defaults/install.conf (same dir as this script) for prompt defaults
defaults_install() {
  local f dir line
  dir=$(cd "$(dirname "$0")" && pwd)
  for f in "$dir/defaults/install.conf" "scripts/install/defaults/install.conf"; do
    [ -f "$f" ] || continue
...
phase_host() {
  ...
  DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
    git curl make sudo dnsmasq ufw jq apache2-utils fail2ban sqlite3 python3-yaml tmux >>"$LOG" 2>&1 || fail apt
  ...
  # Single operator account: root. No extra sudo user is created — the
  # install runs entirely as root and root@$NODE_NAME is the only way in.
  # SSH hardening (password auth off, root key-only) is applied ONLY when a
  # usable authorized key already exists; never lock the operator out of a
  # box that has no keys yet.
  mkdir -p /root/.ssh
  chmod 700 /root/.ssh
  if [ -s /root/.ssh/authorized_keys ]; then
    ...
    fail ssh_keys
    log "no SSH key in /root/.ssh/authorized_keys — add one, then re-check"
  fi
...
problem() {
  case "$1" in
    ...
    ssh_keys)       echo "no SSH keys for root or root" ;;
