#!/usr/bin/env bash
# scripts/render.sh — turn a SKELETON file into a deployable one.
#
...
# written by scripts/install.sh (or by hand — it is plain KEY=VALUE).
#
...
[ -f "$CONF" ] || {
  ...
  echo "  Create it (scripts/install.sh writes it) with the values for THIS host:" >&2
