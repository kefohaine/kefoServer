#!/usr/bin/env bash
#
# scripts/info/goose-tokens.sh — apply config/goose/token-policy.yaml to the live
...
#
# Usage:
#   scripts/info/goose-tokens.sh           apply the policy (backs up the live file)
#   scripts/info/goose-tokens.sh --check   report drift only, change nothing
...
case "$mode" in
  apply|--check) ;;
  *) scripts/lib/mklog error "usage: $0 [--check]"; exit 2 ;;
esac

[ -s "$policy" ] || { scripts/lib/mklog error "missing policy file: $policy"; exit 1; }
[ -s "$briefing" ] || { scripts/lib/mklog error "missing briefing file: $briefing"; exit 1; }
...
case "$rc" in
  0) ;;
  *) scripts/lib/mklog error "goose-tokens: could not merge the policy into $live"; exit 1 ;;
esac

backup="$(printf '%s\n' "$PY_OUT" | awk '/^BACKUP /{print $2}')"
status="$(printf '%s\n' "$PY_OUT" | awk '/^IN_SYNC|^APPLIED|^DRIFT/{print; exit}')"

[ -n "$backup" ] && scripts/lib/mklog info "live goose config backed up to $backup"

case "$status" in
  IN_SYNC*) scripts/lib/mklog info "goose token policy already in sync in $live" ;;
  DRIFT*)   scripts/lib/mklog warn "goose token policy drifted: ${status#DRIFT }"; exit 1 ;;
  APPLIED*) scripts/lib/mklog info "applied: ${status#APPLIED }" ;;
  *)        scripts/lib/mklog warn "goose-tokens: unexpected result: $PY_OUT"; exit 1 ;;
esac
...
if [ "$mode" = apply ]; then
  ...
  if sudo test -s "$env_file"; then
    if ! sudo grep -qxF "$env_line" "$env_file"; then
      printf '%s\n' "$env_line" | sudo tee -a "$env_file" >/dev/null &&
        scripts/lib/mklog info "added GOOSE_MOIM_MESSAGE_FILE to $env_file (restart goose to apply)"
    fi
  else
    scripts/lib/mklog warn "$env_file is missing (run make install-config first) — the serve unit has no briefing"
  fi

  profile=/etc/profile.d/goose-token-caps.sh
  tmp="$(mktemp)"
  {
    echo "# GENERATED by scripts/info/goose-tokens.sh — do not edit."
  ...
  if ! sudo cmp -s "$tmp" "$profile" 2>/dev/null; then
    sudo install -m 0644 "$tmp" "$profile" &&
      scripts/lib/mklog info "wrote $profile"
  fi
  rm -f "$tmp"
fi
