#!/usr/bin/env bash
# scripts/instance.sh — the instance values, for SCRIPTS.
#
# Templates (config/**, services/caddy/**, docs/**, compose files) carry
# {{TOKENS}} or ${VARS} and are rendered by scripts/render.sh. Scripts are CODE:
...
: "${REPO_DIR:=$(cd "$(dirname "$_INSTANCE_SELF")/.." && pwd)}"
