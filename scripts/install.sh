#!/bin/bash
# kefoserver install.sh — root-only, plug-and-play new-VPS installer.
#
...
#
# Entry point: root@kefoserver is the ONLY way in (key-only SSH over the
# tailnet). The installer runs entirely as root — no sudo user, no hand-off.
#
# Usage:
#   scp scripts/install.sh root@kefoserver:
#   ssh root@kefoserver
#   bash install.sh
#
# Paths: the repo lives at $REPO (/root/github/kefoserver); every external,
# untracked artefact (data, modules, databases, uploads) lives under $DATA
# (/root/github/kefoserver/data), which .gitignore excludes.
...
# Instance state and logs live under ONE 'kefohaine' namespace, outside the
# repo: /var/lib/kefohaine (state — it holds the CF token + Tailscale key, so
# it stays root-only 0700, never a world-writable sticky dir like /var/tmp) and
# /var/log/kefohaine (EVERY log this project writes: install, uninstall and any
...
# Consumed by scripts/smoke-vhosts.sh + scripts/fetch.sh.
write_modules_conf() {
  ...
  for m in cloud vault mail monitor; do
    ...
    case "$m" in
      cloud)   ct=$(docker ps --format '{{.Names}}' 2>/dev/null | grep -cx nextcloud)
               [ -f "$REPO/services/nextcloud/docker-compose.yml" ] && cf=1
               [ -e "$DATA/cloud/users" ] && dd=1
               ls "$REPO"/services/*/vhosts/cloud.*.caddy >/dev/null 2>&1 && vh=1 ;;
      vault)   ct=$(docker ps --format '{{.Names}}' 2>/dev/null | grep -cx vaultwarden)
               [ -f "$REPO/services/vaultwarden/docker-compose.yml" ] && cf=1
               [ -e "$DATA/vault/data" ] && dd=1
               ls "$REPO"/services/*/vhosts/vault.*.caddy >/dev/null 2>&1 && vh=1 ;;
      ...
      monitor) ct=$(docker ps --format '{{.Names}}' 2>/dev/null | grep -cx uptimekuma)
               ...
               ls "$REPO"/services/*/vhosts/kuma.*.caddy >/dev/null 2>&1 && vh=1 ;;
    esac
    ev=$((ct+cf+dd+vh))
    [ "$ev" -gt 0 ] && echo "$m" >> "$det"
...
phase_host() {
  ...
  # One-time relocation off the pre-kefohaine paths (never drop the old log).
  if [ ! -f "$LOG" ]; then
    for old in /var/log/kefoserver-install.log /var/log/homelab-install.log; do
      [ -f "$old" ] && { mv -f "$old" "$LOG"; break; }
    done
  fi
  if [ ! -f "$STATE" ] && [ -f /var/lib/kefoserver/setup.state ]; then
    mv -f /var/lib/kefoserver/setup.state "$STATE"
  fi
  ...
  # install runs entirely as root and root@kefoserver is the only way in.
...
EOF
  ...
  if ! tailscale ip -4 >/dev/null 2>&1; then
    # --hostname makes the tailnet node 'kefoserver', so the sole entry point
    # is root@kefoserver (MagicDNS) regardless of the local machine hostname.
    tailscale up --authkey="$TS_AUTHKEY" --hostname=kefoserver >>"$LOG" 2>&1 || fail tailscale_up
  fi
...
ensure_repo() {
  if [ -d "$REPO/.git" ]; then
    ...
    git -C "$REPO" remote set-url origin https://github.com/kefohaine/kefoserver.git 2>/dev/null || true
    log "repo present at $REPO"
    return
  fi
  mkdir -p /root/github
  # Public repo: clone over HTTPS — no SSH key has to be added to GitHub.
  if git clone https://github.com/kefohaine/kefoserver.git "$REPO" >>"$LOG" 2>&1; then
    git -C "$REPO" remote set-url origin https://github.com/kefohaine/kefoserver.git
  else
...
prep_dirs() {
  ...
  mkdir -p /root/github/kefoserver/data/cloud/html \
               /root/github/kefoserver/data/cloud/users \
               /root/github/kefoserver/data/vault/data \
               /root/github/kefoserver/data/kuma/data \
               /root/github/kefoserver/data/mailserver/data \
               /root/github/kefoserver/data/mailserver/state \
               /root/github/kefoserver/data/mailserver/logs \
               /root/github/kefoserver/data/mailserver/config \
...
caddy_data() {
  ...
  sudo mkdir -p /root/github/kefoserver/data/caddy_data
  printf 'CF_API_TOKEN=%s\n' "$CF_API_TOKEN" | sudo tee /root/github/kefoserver/data/caddy_data/CF_API_TOKEN >/dev/null
  sudo chown -R 201:201 /root/github/kefoserver/data/caddy_data
  ...
  sudo chmod 0711 /root/github/kefoserver/data/caddy_data
  sudo chown root:root /root/github/kefoserver/data/caddy_data/CF_API_TOKEN
  sudo chmod 0644 /root/github/kefoserver/data/caddy_data/CF_API_TOKEN
}
...
finalize_hardening() {
  ...
  if [ -n "$ip" ] && [ "$ip" != "$(tailscale ip -4 2>/dev/null | head -1)" ]; then
    ...
    log "  removed the temporary installer SSH allow for $ip (root@kefoserver over the tailnet remains the entry point)"
  fi
}
...
success_block() {
  ...
  echo ""
  echo " SSH: root@kefoserver is the only entry point (key-only, tailnet-only, port 22)."
  echo ""
...
main() {
  if [ "$(id -u)" -ne 0 ]; then
    echo "Run as root: root@kefoserver is the only entry point (no sudo hand-off)."; exit 1
  fi
  banner
  load_state
  ask_inputs
