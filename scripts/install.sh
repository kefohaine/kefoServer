...
#
# Usage:
#   scp scripts/install.sh root@$NODE_NAME:
#   ssh root@$NODE_NAME
#   bash install.sh
#
...
write_instance_conf() {
  ...
  # are rendered from it (make render) and scripts read it (scripts/instance.sh).
  ...
  [ -f "$f" ] && keep=$(grep -E '^(GITHUB_USER|SERVER_IP|TAILNET_SUBNET|EMAIL|TIMEZONE)=' "$f" || true)
  {
    ...
    echo "# Written by scripts/install.sh; see config/instance.defaults for what"
  ...
  bash "$REPO/scripts/render-all.sh" >>"$LOG" 2>&1 || fail render
}
...
recheck() {
  case "$1" in
    ...
    render) bash "$REPO/scripts/render-all.sh" >/dev/null 2>&1 ;;
