...
# State holds the CF token + Tailscale key: keep it in a root-only dir (not a
# world-writable sticky dir like /var/tmp) and mode 0600.
STATE=/var/lib/kefoserver/setup.state
LOG=/var/log/kefoserver-install.log
ERR_TAGS=()
...
hint() {
  case "$1" in
    ...
    panel_admin)    echo "the automatic admin insert failed — inspect /var/log/kefoserver-install.log; fallback: create the admin in the UI at https://mc.$DOMAIN/panel, or run: sudo sqlite3 puffer/data/pufferpanel.db \"INSERT INTO users ...\" per docs/GUIDE.md 'Admin login recovery', then re-check" ;;
