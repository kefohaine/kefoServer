#!/usr/bin/env bash
# scripts/ttyd-devices.sh — per-tailnet-device shells, listed on the tail page.
#
#   scripts/ttyd-devices.sh list                 what exists now
#   scripts/ttyd-devices.sh add <name>           create a shell + list it
#   scripts/ttyd-devices.sh rm  <name>           kill it + unlist it
#
...
cmd_add() {
  local raw="${1:-}" n
  [ -n "$raw" ] || { echo "usage: scripts/ttyd-devices.sh add <name>" >&2; exit 1; }
  n="$(sanitise "$raw")"
  ...
  bash "$ROOT/scripts/tail-targets.sh" >/dev/null 2>&1 && echo "info:  tail page catalogue refreshed (make tail-targets)"
  echo "info:  open https://tail.$DOMAIN/ttyd?arg=$n"
}

cmd_rm() {
  local raw="${1:-}" n
  [ -n "$raw" ] || { echo "usage: scripts/ttyd-devices.sh rm <name>" >&2; exit 1; }
  n="$(sanitise "$raw")"
  if names | grep -qx "$n"; then
  ...
  bash "$ROOT/scripts/tail-targets.sh" >/dev/null 2>&1 && echo "info:  tail page catalogue refreshed"
}
...
case "${1:-list}" in
  ...
  *) echo "usage: scripts/ttyd-devices.sh [list|add <name>|rm <name>]" >&2; exit 2 ;;
esac
