...
#
# Output: modules/<domain>/www/targets.json (GENERATED — do not hand-edit).
...
#     `make ttyd-add/ttyd-rm` (scripts/access/ttyd-devices.sh). No vhost, no port.
set -uo pipefail
...
pages_out, vhosts = [], 0
for path in sorted(glob.glob(os.path.join(os.environ.get("REPO_DIR","."),"modules/caddy/vhosts/*.caddy"))):
