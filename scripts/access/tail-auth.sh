#!/usr/bin/env bash
#
# scripts/access/tail-auth.sh — manage the tail.$DOMAIN HTTP basic-auth credential.
#
...
#   user defaults to `$GITHUB_USER`; the password is generated when omitted. The script
...
VHOST="$(ls modules/caddy/vhosts/tail.caddy 2>/dev/null | head -1)"
[ -n "$VHOST" ] || { echo "error: no modules/caddy/vhosts/tail.caddy found (run from the repo root)"; exit 1; }
